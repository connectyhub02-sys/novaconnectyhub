...
const mock = vi.hoisted(() => ({ user: "admin-user" as string | null, admin: true, auditFails: false, audit: vi.fn(), rpc: vi.fn(), eq: vi.fn() }));
vi.mock("server-only", () => ({}));
...
beforeEach(() => {
  vi.clearAllMocks(); mock.user = "admin-user"; mock.admin = true; mock.auditFails = false;
