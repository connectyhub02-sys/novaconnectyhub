...
function setup(oneTime=false, discounted=false) {
 ...
 const requireAddress=vi.fn(async()=>({postalCode:'01001000',number:'10'}));
...
describe('panel native card orchestration',()=>{
 ...
 it('charges the welcome price while authorizing the full recurring price',async()=>{const s=setup(false,true);await s.pay();expect(s.adapter.createAsaasDirectCardPayment).toHaveBeenCalledWith(expect.objectContaining({amount:40}));expect(s.rpc.mock.calls.find(call => call[0] === 'claim_native_billing_card')![1]).toMatchObject({p_amount:40,p_recurring:110});});
 ...
 it('charges the first total once and saves an authorization without an external subscription',async()=>{
  ...
  expect(s.rpc.mock.calls.some(call => call[0] === 'claim_native_billing_card')).toBe(true);expect(JSON.stringify(s.rpc.mock.calls)).not.toContain(card.number);expect(JSON.stringify(s.rpc.mock.calls)).not.toContain('ccv');
 });
 ...
 it('one-time plan charges once without creating an agreement or requiring renewal consent',async()=>{
  ...
  expect(t.rpc.mock.calls.find(call => call[0] === 'claim_native_billing_card')![1].p_recurring).toBe(0);expect(t.saveCard).not.toHaveBeenCalled();
 });
