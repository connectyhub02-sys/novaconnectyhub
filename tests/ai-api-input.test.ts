...
const api=serverModuleHarness<{parseAiInput:(raw:unknown,max:number)=>{maxTokens:number;providerBody:{contents:unknown[]}};createAiSecret:()=>{secret:string;key_hash:string};hashAiSecret:(s:string)=>string}>("src/lib/ai-api/gateway.ts",{"./advanced-input":advancedInput,"node:crypto":crypto});
...
it("rejects remote URLs, empty tool declarations and oversized output",()=>{expect(()=>api.parseAiInput({messages:[{role:"user",content:[{type:"image_url",image_url:{url:"http://127.0.0.1/secret"}}]}]},100)).toThrow("URLs remotas");expect(()=>api.parseAiInput({messages:[{role:"user",content:"Oi"}],tools:[]},100)).toThrow("ferramentas");expect(()=>api.parseAiInput({messages:[{role:"user",content:"Oi"}],max_tokens:101},100)).toThrow("configuração de resposta");});
