...
it("rejects remote URLs, hidden tools and oversized output",()=>{expect(()=>api.parseAiInput({messages:[{role:"user",content:[{type:"image_url",image_url:{url:"http://127.0.0.1/secret"}}]}]},100)).toThrow("URLs remotas");expect(()=>api.parseAiInput({messages:[{role:"user",content:"Oi"}],tools:[]},100)).toThrow("Parâmetro");expect(()=>api.parseAiInput({messages:[{role:"user",content:"Oi"}],max_tokens:101},100)).toThrow("configuração de resposta");});
