...
function fixture(role="owner") {
  ...
  const rpc=vi.fn(async(name:string)=>({data:name==="create_ai_project_with_model_key"?"project-id":{totals:{requests:0}},error:null}));
  ...
  const route=serverModuleHarness<{GET:(request:Request)=>Promise<Response>;POST:(request:Request)=>Promise<Response>}>("src/app/api/dashboard/ai/route.ts",{
    ...
    "@/lib/billing/contract-access":{getContractAccess:async()=>({billing_organization_id:"account-org",plan_code:"scale"})},
    "@/lib/billing/access-control":{assertOrganizationOperationalAccess:async()=>({planCode:"scale"})},
    "@/lib/ai-api/gateway":{record:(value:unknown)=>value,createAiSecret:()=>({secret:"fake-secret",key_hash:"hash",key_prefix:"prefix"})},
    "@/lib/ai-api/model-service":{loadPublicAiModels:async()=>[],assertAiModelAvailable:async()=>({id:"flash-3.5"})},
    "@/lib/ai-api/model-catalog":{recommendedAiModel:"flash-3.5"},
    "@/lib/ai-api/usage":{aiUsagePeriod,aiUsageStart},
  });
  return {route,rpc,selections};
}
describe("Simple AI dashboard boundary",()=>{
  it("creates from a name with the recommended model and resolves the billing account on the server",async()=>{
    ...
    expect(f.rpc).toHaveBeenCalledWith("create_ai_project_with_model_key",{p_org:"account-org",p_name:"My app",p_hash:"hash",p_prefix:"prefix",p_model:"flash-3.5"});
  ...
  it("aggregates by the authenticated account and does not select internal usage fields",async()=>{
    ...
    expect(f.selections.join(" ")).not.toMatch(/\*|token|provider|metadata|key_hash|limit/);
  });
});
