...
async function request(url,{method='GET',token,body,raw}={}){const u=new URL(url);assert(['auth','kong','managed-gateway','objects','objects-pressure','telemetry-gateway'].includes(u.hostname),'Unmapped endpoint refused');if(local){assert(local[u.hostname],'Unmapped native endpoint refused');url=new URL(u.pathname+u.search,local[u.hostname]);}else{u.hostname='127.0.0.1';url=u;}const r=await fetch(url,{method,headers:{...(token?{Authorization:`Bearer ${token}`}:{ }),...(body?{'Content-Type':'application/json'}:{})},body:raw??(body?JSON.stringify(body):undefined),redirect:'error',signal:AbortSignal.timeout(10000)});const bytes=Buffer.from(await r.arrayBuffer());let data;try{data=JSON.parse(bytes.toString());}catch{data=undefined;}return {status:r.status,ok:r.ok,data,bytes};}
