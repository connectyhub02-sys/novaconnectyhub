...
export function createAiRelay({controlUrl,secret,fetcher=fetch,Socket=WebSocket,maxSessions=16,maxSessionMs=900000,uploadsEnabled=false,uploadStateDir}) {
  ...
  const server=createServer(async(req,res)=>{
    ...
    const health=req.method==='GET'&&['/','/health'].includes(req.url);res.writeHead(health?200:404,{'Content-Type':'application/json'});res.end(JSON.stringify(health?{ok:true,version:'2026-09-14',live:true,uploads:uploadsEnabled}:{error:'not_found'}));
  });
  server.requestTimeout=120000;
  const sockets=new WebSocketServer({server,maxPayload:2_000_000,perMessageDeflate:false});
  const sessions=new Set();
  const closeServer=server.close.bind(server);
  server.close=(...args)=>{upload?.close();for(const close of sessions)close();return closeServer(...args);};
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href) {
  const server=createAiRelay({controlUrl:process.env.AI_RELAY_CONTROL_URL,secret:process.env.AI_RELAY_SECRET,uploadsEnabled:process.env.AI_UPLOAD_RELAY_ENABLED==='true',uploadStateDir:process.env.AI_UPLOAD_STATE_DIR});
