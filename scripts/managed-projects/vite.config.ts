...
export default defineConfig({root:here,resolve:{alias:[
 ...
 const projects=api.unwrap(await session.db.from('managed_projects').select('*').order('name',{ascending:true}).limit(200));let snapshot;let samples;let settings;let alertEvents;let alertStates;
 if(path[0]==='projetos'&&path[1])snapshot=await api.projectSnapshot(session.db,path[1]);
 if(path[0]==='vps'){api.requireInfrastructure(session.admin);samples=api.unwrap(await session.db.from('infrastructure_samples').select('*').order('measured_at',{ascending:false}).limit(120));settings=api.unwrap(await session.db.from('infrastructure_alert_settings').select('*').eq('id',true).single());alertEvents=api.unwrap(await session.db.from('infrastructure_alert_events').select('*').order('measured_at',{ascending:false}).limit(100));alertStates=api.unwrap(await session.db.from('infrastructure_alert_states').select('*'));}
 response=Response.json({identity,admin:session.admin,userId:session.userId,projects,companies,snapshot,path,samples,settings,alertEvents,alertStates});
 }else{
