...
export function BillingPaymentMethods({subscriptions}:{subscriptions:Subscription[]}){
  const active=subscriptions.filter(s=>["active","past_due"].includes(s.status));
  ...
  async function load(){if(subscriptionId){const version=++loadVersion.current;const next=await fetchCards(subscriptionId);if(version===loadVersion.current)setData(next);}}
  useEffect(()=>{
    let disposed=false;
    const timer=setTimeout(()=>{
      setData(null);setChoice(null);setError('');
      const version=++loadVersion.current;
      if(subscriptionId)void fetchCards(subscriptionId).then(next=>{if(!disposed&&version===loadVersion.current)setData(next);}).catch(e=>{if(!disposed)setError(e.message);});
    },0);
    // Request sequence counter: invalidate in-flight refreshes on cleanup.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    return()=>{disposed=true;loadVersion.current++;clearTimeout(timer);};
  },[subscriptionId]);
  useEffect(()=>{
    if(choice!=="add")return;
    let disposed=false;
    void fetch('/api/dashboard/billing/address',{cache:'no-store'}).then(async response=>{if(!response.ok)return;const profile=await response.json();if(!disposed)setHolderDefaults({...profile.contact,postalCode:profile.address?.postalCode??'',addressNumber:profile.address?.number??''});}).catch(()=>{});
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{const r=await fetch("/api/dashboard/billing/payment-methods",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const j=await r.json();
      ...
      setMessage(j.message);setChoice(null);window.dispatchEvent(new Event("connectyhub:billing-refresh"));await load();
  ...
  return <section id="metodos-pagamento" aria-labelledby="payment-methods-title" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    {data?<>
      ...
      {data.cards.length?<ul className="mt-4 grid gap-3 md:grid-cols-2">{[...data.cards].sort((a,b)=>Number(b.status==="active")-Number(a.status==="active")).map(card=><li key={card.id} className={`flex flex-col items-start justify-between gap-3 rounded-lg border p-4 sm:flex-row sm:items-center ${card.status==="active"?"border-blue-200 bg-blue-50/60":"border-slate-200"}`}><div><p className="font-semibold text-slate-950">{card.brand??"Cartão"} {card.last_digits?`•••• ${card.last_digits}`:"com identificação indisponível"} {card.status==="active"?<span className="ml-2 rounded-full bg-blue-100 px-2 py-1 text-xs text-blue-800">Padrão</span>:null}</p><p className="mt-1 text-xs text-slate-600">{card.exp_month&&card.exp_year?`Validade ${card.exp_month}/${card.exp_year}`:"Dados parciais não disponíveis para este cartão antigo."}</p>{card.status==="active"?<p className="mt-2 text-xs font-medium text-blue-800">A próxima renovação usará este cartão.</p>:null}</div>{data.cards.filter(card => isCardWithinValidity(card)).length>1&&isCardWithinValidity(card)&&card.status!=="active"&&!data.blocker?<button disabled={busy} className="min-h-11 w-full shrink-0 rounded-lg border border-slate-300 px-3 text-sm font-semibold text-blue-800 sm:w-auto disabled:opacity-50" onClick={()=>{setChoice(card.id);setError("");setMessage("");}}>Usar na próxima renovação</button>:null}</li>)}</ul>:!data.blocker?<p className="mt-4 text-sm text-slate-600">Nenhum cartão de renovação cadastrado. Adicione um cartão para autorizar as próximas renovações.</p>:null}
    ...
    {choice==="add"&&data&&!data.blocker&&data.cards.some(c=>c.status==="active")&&subscription?<BillingCardReplacement key={subscription.id} subscriptionId={subscription.id} planName={subscription.planName} onSaved={()=>{setMessage("Novo cartão padrão confirmado. Confira os dados do titular em Faturamento e consumo.");void load().catch(e=>setError(e.message));}} onClose={()=>{setChoice(null);void load().catch(e=>setError(e.message));}}/>:null}
