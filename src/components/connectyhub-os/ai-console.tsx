...
type Project = { id: string; name: string; status: string; ai_api_keys: Array<{ id: string; name: string; key_prefix: string; status: string }> };
type Activity = { id: string; project_id: string; status: string; charged_credits: number; reserved_credits: number; created_at: string };
type Data = { projects: Project[]; wallet: { balance_credits: number; reserved_credits: number } | null; activity: Activity[]; usage: AiUsage };
  ...
  return <div className="mx-auto max-w-6xl space-y-6 text-slate-900">
    ...
    {tab==="usage"&&<>
      ...
      {error?<p role="alert" className="rounded-xl bg-amber-50 p-5 text-sm text-amber-900">{error}</p>:!data?<p role="status" className="py-12 text-center text-sm text-slate-500">Carregando seu uso…</p>:<>
        {!data.projects.length&&<div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-dashed border-blue-300 bg-white p-6"><div><h2 className="font-bold">Conecte seu primeiro projeto</h2><p className="mt-2 text-sm text-slate-600">Você só precisa dar um nome. A chave é criada junto com o projeto.</p></div><button className={primary} onClick={()=>setTab("projects")}><Plus size={16}/>Criar meu projeto</button></div>}
    ...
    {tab==="projects"&&<>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="text-lg font-bold">Novo projeto</h2><p className="mt-2 text-sm leading-7 text-slate-500">Dê um nome para identificar o sistema que vai usar sua chave.</p><form className="mt-4 flex flex-wrap items-end gap-3" onSubmit={event=>{event.preventDefault();void action({action:"create_project",name:new FormData(event.currentTarget).get("name")});}}><label className="min-w-0 flex-1 text-sm font-medium">Nome do projeto<input required name="name" maxLength={100} placeholder="Meu aplicativo" className={field}/></label><button disabled={busy} className={primary}><Plus size={16}/>{busy?"Criando…":"Criar projeto e chave"}</button></form></section>
      {error&&<p role="alert" className="rounded-xl bg-amber-50 p-4 text-sm">{error}</p>}
      <section className="space-y-3"><h2 className="text-lg font-bold">Meus projetos</h2>{state?.data.projects.map(item=><article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="break-words font-bold">{item.name}</h3><p className="mt-1 text-sm text-slate-500">{item.status==="active"?"Pronto para usar":"Pausado"}</p></div><div className="flex flex-wrap gap-2"><button disabled={busy} className={primary} onClick={()=>action({action:"create_key",projectId:item.id})}><KeyRound size={16}/>Nova chave</button><button disabled={busy} className="min-h-11 rounded-xl border border-slate-300 px-3 text-sm" onClick={()=>action({action:"toggle_project",projectId:item.id,status:item.status==="active"?"paused":"active"})}>{item.status==="active"?"Pausar":"Ativar"}</button></div></div><details className="mt-3"><summary className="cursor-pointer py-3 text-sm font-semibold text-blue-800">Renomear projeto</summary><form className="flex flex-wrap gap-3" onSubmit={event=>{event.preventDefault();void action({action:"update_project",projectId:item.id,name:new FormData(event.currentTarget).get("name")});}}><input aria-label={`Nome de ${item.name}`} name="name" required maxLength={100} defaultValue={item.name} className={`${field} min-w-0 flex-1`}/><button disabled={busy} className={primary}>Salvar nome</button></form></details>{item.ai_api_keys.map(value=><div key={value.id} className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 p-3 text-xs"><code className="break-all">{value.key_prefix}•••• · {value.status==="active"?"Ativa":"Revogada"}</code>{value.status==="active"&&<button disabled={busy} className="min-h-11 px-2 font-semibold text-red-700" onClick={()=>action({action:"revoke_key",projectId:item.id,keyId:value.id})}>Revogar chave</button>}</div>)}</article>)}</section>
