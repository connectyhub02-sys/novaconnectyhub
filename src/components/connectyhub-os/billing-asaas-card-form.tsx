"use client";
import { managedRenewalConsent, managedRenewalConsentVersion } from "@/lib/billing/managed-renewal-policy";
...
export function BillingAsaasCardForm({ subscriptionId, cartSyncing, onBusyChange, onStatusChange }: { subscriptionId: string; cartSyncing: boolean; onBusyChange: (busy: boolean) => void; onStatusChange: (result: CardPaymentStatusChange) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ attemptId: crypto.randomUUID(), amount: quote.amount, revision: quote.revision, card, holder, acceptRecurring: form.get("recurring") === "on", recurringConsentVersion: managedRenewalConsentVersion }) });
  ...
  const needsHolder = editHolder || !quote.holder.name || !quote.holder.email || !quote.holder.cpfCnpj || !quote.holder.phone;
  return <form ref={formRef} onSubmit={submit} data-sensitive="payment" className="mt-6 space-y-5 border-t border-slate-200 pt-5 text-slate-900">
    <div><h3 className="font-bold">Dados do cartão</h3><p className="mt-1 text-xs text-slate-600">Hoje: {money(quote.amount)}. {quote.recurringAmount > 0 ? `Renovação ${quote.recurrenceLabel.toLowerCase()}: ${money(quote.recurringAmount)}. Adicionais avulsos não entram nas renovações.` : "Pagamento único, sem renovação automática."}</p></div>
    {waiting ? <p role="status" className="rounded-lg bg-blue-50 p-3 text-sm">Estamos conferindo o resultado do pagamento. Você receberá a atualização pelo WhatsApp.</p> : <fieldset disabled={sending || cartSyncing} className="space-y-4">
      ...
      {needsHolder ? <div className="grid gap-3 sm:grid-cols-2">{([{ key: "name", label: "Nome completo" }, { key: "email", label: "E-mail" }, { key: "cpfCnpj", label: "CPF/CNPJ" }, { key: "phone", label: "Telefone" }] as const).filter(f => editHolder || !quote.holder[f.key]).map(f => <label key={f.key} className="text-xs font-semibold">{f.label}<input className={field} required value={holder[f.key]} type={f.key === "email" ? "email" : "text"} onChange={e => setHolder({ ...holder, [f.key]: e.target.value })} /></label>)}</div> : null}
