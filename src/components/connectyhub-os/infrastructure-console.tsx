...
import { actionLabels, blockedReason, eventLabels, healthLabels, migrationRiskLabels, originLabels, projectHealth, stageLabels, stages, stale, type Detail, type Health, type InfraAction, type Migration, type MigrationExecution, type Overview, type Telemetry } from "@/lib/infrastructure/model";
...
export function InfrastructureProject({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetch(`${base}/migrations`, { signal: controller.signal, cache: "no-store" }).then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error); setCatalog(d.migrations); setExecution(d.execution); }).catch(e => { if (!controller.signal.aborted) setCatalogError(e.message); });
  ...
  return <main className="mx-auto max-w-7xl space-y-6 text-slate-900">
    ...
    {data && <>
      ...
      {tab === "migrations" && <Section title="Migrations versionadas"><p className="text-sm leading-6 text-slate-600">O catálogo é versionado e cada SQL recebe pré-validação, checksum e resumo de risco antes de qualquer solicitação.</p>{execution && <div className={`rounded-xl p-4 text-sm ${execution.status === "ready" ? "bg-emerald-50 text-emerald-900" : "bg-amber-50 text-amber-900"}`}><strong>{execution.status === "ready" ? "Executor configurado" : "Ação preparada · execução bloqueada"}</strong><p className="mt-1">{execution.status === "ready" ? "Ainda exige confirmação forte e auditoria." : `Faltam: ${execution.missing.join(", ")}`}</p></div>}{catalogError && <p role="alert" className="text-sm text-rose-800">{catalogError}</p>}{!catalog && !catalogError && <p>Carregando catálogo…</p>}{catalog?.length === 0 && <p className="text-sm text-slate-500">Catálogo deste projeto ainda não conectado. Nenhuma migration é presumida aplicada.</p>}
        ...
        {catalog?.filter(m => m.name.toLowerCase().includes(migrationFilter.toLowerCase())).map(m => <details key={m.version} className="rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer break-all text-sm font-semibold"><Database className="mr-2 inline" size={14} />{m.name}<span className="ml-3 text-xs font-normal text-slate-500">{stale(telemetry) || !snapshot?.appliedMigrations ? "Aplicação não verificada" : snapshot.appliedMigrations.includes(m.version) ? "Aplicada (informada pelo coletor)" : "Pendente (ausente no snapshot)"}</span></summary><div className="my-3 flex flex-wrap gap-2 text-xs"><span className={`rounded-full px-3 py-1 font-semibold ${m.risk.level === "destructive" ? "bg-rose-100 text-rose-800" : m.risk.level === "review" ? "bg-amber-100 text-amber-900" : "bg-emerald-100 text-emerald-800"}`}>{migrationRiskLabels[m.risk.level]}</span><span className="rounded-full bg-slate-100 px-3 py-1">{m.risk.transactional ? "Transação possível" : "Fora de transação"}</span></div>{m.risk.reasons.length > 0 && <ul className="mb-3 list-disc pl-5 text-xs text-amber-900">{m.risk.reasons.map(reason => <li key={reason}>{reason}</li>)}</ul>}<p className="break-all text-xs text-slate-500">SHA-256: {m.checksum}</p><pre className="my-3 max-h-80 overflow-auto rounded-xl bg-slate-950 p-4 text-xs text-slate-200">{m.sql}</pre>{actionButton("migration_apply", m.version, m.checksum)}</details>)}
      </Section>}
