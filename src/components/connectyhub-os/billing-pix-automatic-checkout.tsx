...
export function BillingPixAutomaticCheckout({ subscriptionId, initial, onChange, cartSyncing }: { subscriptionId: string; initial: PixAutomaticSnapshot; onChange: (snapshot: PixAutomaticSnapshot) => void; cartSyncing: boolean }) {
  ...
  return <section className="mt-4 space-y-3 rounded-xl border border-slate-300 bg-white p-4 text-slate-900">
    ...
    {authorization?.active ? <p role="status" className="rounded-lg bg-emerald-50 p-3 text-sm text-emerald-800">Pagamento confirmado e Pix Automático ativo. Seu plano está sendo atualizado.</p> : authorization && !authorization.canRetry ? <>
      <p role="status" className="text-sm">{authorization.state === "CREATED" ? "No app do seu banco, escaneie o QR Code ou use Pix Copia e Cola. Confira e confirme o primeiro pagamento e a autorização recorrente. Depois da confirmação, os próximos ciclos serão pagos automaticamente conforme autorizado." : ["CANCELLED", "EXPIRED"].includes(authorization.state) ? "A autorização foi encerrada. Confira com a equipe antes de iniciar outro pagamento." : "Aguardando confirmação do banco. Não faça outro pagamento enquanto conferimos."}</p>
      ...
      <p className="text-sm text-slate-600">Ao gerar o Pix Automático, a ConnectyHub exibirá o QR Code e o Pix Copia e Cola do primeiro pagamento. Use um deles no app do seu banco para confirmar o pagamento e autorizar a recorrência. Com a autorização ativa, os próximos ciclos seguirão automaticamente.</p>
