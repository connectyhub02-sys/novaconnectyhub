...
export function BillingAddressEditor({ onReadyChange, subscriptionId }: { onReadyChange?: (ready: boolean) => void; subscriptionId?: string }) {
  ...
  async function save() {
    ...
    try {
      ...
      lookupVersion.current++; setLookingUp(false); setAddress(confirmed); setSaved(confirmed); setEditing(false);
  ...
  const input = (key: keyof BillingAddress, label: string, options: { autoComplete?: string; maxLength?: number; wide?: boolean; optional?: boolean } = {}) => <label className={`block text-xs font-medium text-slate-700 ${options.wide ? "sm:col-span-2" : ""}`} htmlFor={`${prefix}-${key}`}>
  ...
  return <section aria-label="Endereço de faturamento" className="mt-6 border-t border-slate-200 pt-5">
    ...
    {loading ? <p role="status" className="mt-3 text-sm text-slate-500">Carregando endereço…</p> : editing ? <>
      ...
      <div className="mt-4 flex flex-wrap gap-3"><button type="button" disabled={saving || lookingUp} onClick={() => void save()} className="min-h-11 rounded-lg bg-blue-700 px-4 text-sm font-semibold text-white disabled:opacity-60">{saving ? "Salvando…" : "Salvar endereço"}</button>{saved ? <button type="button" disabled={saving} onClick={() => { lookupVersion.current++; setLookingUp(false); setAddress(saved); setEditing(false); setMessage(""); }} className="min-h-11 px-3 text-sm text-slate-600">Cancelar edição</button> : null}</div>
