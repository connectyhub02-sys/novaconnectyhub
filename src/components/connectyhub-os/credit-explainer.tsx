...
export function CreditExplainer({ compact = false }: { compact?: boolean }) {
  return <details className="rounded-2xl border border-blue-200 bg-blue-50 text-slate-900">
    ...
    <div className="space-y-3 px-4 pb-4 text-sm leading-6 text-slate-700">
      ...
      <p>Cada solicitação desconta créditos conforme o trabalho realizado. O painel mostra quanto foi utilizado, em qual projeto e quando. Automações e atendimentos também usam esse saldo.</p>
      {!compact && <div className="rounded-xl bg-white p-3"><strong>Exemplo ilustrativo</strong><p>Saldo de 1.000 → atividade de 3 → saldo de 997 créditos.</p><p className="text-xs text-slate-500">O exemplo não é uma tarifa. Não existe um custo fixo por mensagem; consulte os créditos utilizados nas atividades.</p></div>}
      <Link href="/docs/api#ia-creditos" className="inline-flex min-h-11 items-center gap-2 font-semibold text-blue-800 underline underline-offset-4">Entender meus créditos<ArrowRight size={14}/></Link>
    </div>
  </details>;
}
