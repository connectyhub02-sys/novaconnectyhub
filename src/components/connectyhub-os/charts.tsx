...
export function AreaChartPanel({
  title, eyebrow, value, trend, data, color = "var(--ch-chart-1)", filters, compact = false,
}: {
  ...
  return (
    ...
    >
      ...
      <div className={compact ? "px-3 pb-0 pt-2" : "px-5 pb-1 pt-4"}>
        <p className={cn("font-mono font-bold", compact ? "text-[20px]" : "text-[26px]")} style={{ color }}>{value}</p>
      </div>
      <div className={cn("px-2", compact ? "h-[116px] pb-2" : "h-[180px] pb-3")}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="var(--ch-chart-grid)" strokeOpacity={0.9} vertical={false} />
...
export function BarChartPanel({
  title, eyebrow, data, color = "var(--ch-chart-1)", filters, action, compact = false,
}: {
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }} barSize={compact ? 13 : 18}>
            ...
            <Tooltip
              ...
              labelStyle={{ color: "var(--ch-muted)" }}
              itemStyle={{ color }}
              cursor={{ fill: "rgba(var(--ch-accent-rgb),0.08)" }}
            />
