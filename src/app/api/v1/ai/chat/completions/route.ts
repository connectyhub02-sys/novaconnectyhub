...
import {streamChatAi} from '@/lib/ai-api/chat-stream';
...
export async function POST(request: Request) {
  try {
    ...
    if(body?.stream===true) {
      const result=await streamChatAi(request,body);
      after(async()=>{const organizationId=await result.completion;if(organizationId)await processWalletAlerts(createServiceClient(),organizationId).catch(()=>undefined);});
    ...
    return NextResponse.json({ error: { code: error instanceof AiApiError ? publicAiErrorCode(error.code) : "access_or_service_unavailable", request_id:requestId, message: error instanceof AiApiError ? error.message : "Não foi possível concluir a solicitação. Verifique o acesso e o histórico no painel." } }, { status, headers: { "Cache-Control": "no-store",...(status===429?{'Retry-After':'60'}:{}),...(requestId?{"X-Request-Id":requestId}:{}) } });
  }
}
