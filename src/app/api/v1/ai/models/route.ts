...
export async function GET(request: Request) {
  try {
    ...
    const { models, defaultModel } = await listAiModels();
    return NextResponse.json({ object: "list", data: [{ id: "connectyhub-auto", object: "model", owned_by: "connectyhub", resolves_to: defaultModel }, ...models.map((m) => ({ id: m.provider_model_id, object: "model", owned_by: "connectyhub" }))] }, { headers: { "Cache-Control": "no-store" } });
