...
import { parseBillingContact } from "@/lib/billing/billing-contact";
...
export async function GET() {
  ...
  try { return json(await loadBillingProfile(createServiceClient(), workspace.organization.id)); }
...
export async function PATCH(request: NextRequest) {
  ...
  let address, contact, subscriptionId: string | null, suggestionId: string | null, expectedUpdatedAt: string | null;
  try {
    const body = record(JSON.parse(raw)); address = parseBillingAddress(body);
    contact = body.contact ? parseBillingContact(body.contact) : null;
    ...
    if (expectedUpdatedAt && !Number.isFinite(Date.parse(expectedUpdatedAt))) throw new Error("Atualize os dados de faturamento antes de salvar.");
  }
  ...
  if (suggestionId && !/^[0-9a-f-]{36}$/i.test(suggestionId)) return json({ error: "Sugestão inválida." }, 422);
  const client = createServiceClient();
  const result = await client.rpc("confirm_organization_billing_profile", { p_org: workspace.organization.id, p_actor: workspace.user.id, p_address: address, p_subscription: subscriptionId,
    p_contact: contact, p_expected_updated_at: expectedUpdatedAt, p_suggestion: suggestionId });
  if (result.error?.message.includes("BILLING_PROFILE_CHANGED")) return json({ error: "Os dados de faturamento mudaram em outra tela. Recarregue os dados e confira antes de salvar.", code: "billing_profile_changed" }, 409);
