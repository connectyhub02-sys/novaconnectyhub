...
export default async function ProductPage({ params, searchParams }: ProductPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f9ff] pb-28 text-slate-950 sm:pb-0">
      <script
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-4 sm:px-6 lg:px-8 lg:py-7">
        ...
        <div className="mt-4 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_360px] xl:grid-cols-[minmax(0,1fr)_390px]">
          <section className="min-w-0 overflow-hidden rounded-[8px] border border-blue-100 bg-white shadow-2xl shadow-blue-950/10">
            <div className="grid min-w-0 gap-0 lg:grid-cols-[minmax(0,0.98fr)_minmax(0,1fr)]">
              ...
              <div className="min-w-0 p-5 sm:p-7">
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="rounded-full border border-[#25D366]/30 bg-[#25D366]/10 px-3 py-1 text-xs font-bold text-[#128C4A]">
                ...
                {hasLongDescription ? (
                  <a
                    ...
                    className="mt-4 inline-flex w-fit items-center gap-2 rounded-[8px] border border-blue-100 bg-white px-3 py-2 text-xs font-bold text-blue-700 transition hover:border-[#25D366]/50 hover:text-[#128C4A]"
                    data-track-event="sales_catalog_product_description_preview_clicked"
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-3">
                  ...
                  <ProductFact icon={<MessageCircle className="h-4 w-4" />} label="Compra pelo WhatsApp" />
                  <ProductFact icon={<CheckCircle2 className="h-4 w-4" />} label="Pedido rastreado" />
                </div>

                <div className="mt-6 rounded-[8px] border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-4">
                  <p className="text-xs font-bold uppercase text-blue-700">Valor do produto</p>
                  <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
                    <p className="text-3xl font-black text-slate-950">{priceLabel}</p>
                    {item.offer.callToAction ? (
                      <p className="max-w-[260px] text-sm font-semibold text-[#128C4A]">{item.offer.callToAction}</p>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="mt-4 hidden sm:block">
                  ...
                  <p className="mt-3 text-center text-xs font-medium text-slate-500">
                    Voce continua acompanhado pela loja no WhatsApp apos o pedido.
                  </p>
                </div>
              </div>
            </div>
          ...
          <aside className="grid min-w-0 content-start gap-4">
            {related.length > 0 ? (
              <section className="min-w-0 rounded-[8px] border border-blue-100 bg-white p-4 shadow-xl shadow-blue-950/10 sm:p-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-bold uppercase text-blue-700">Relacionados</p>
                ...
                <div className="mt-4 flex gap-3 overflow-x-auto pb-1 lg:grid lg:overflow-visible lg:pb-0">
                  {related.map((relatedItem) => {
                    ...
                    return (
                      <Link
                        ...
                        className="group flex min-w-[240px] items-center gap-3 rounded-[8px] border border-blue-100 bg-slate-50 p-3 transition hover:border-[#25D366] hover:bg-emerald-50 lg:min-w-0"
                        data-track-event="sales_catalog_similar_product_clicked"
                        data-track-label={relatedItem.title}
                      >
                        <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-[8px] bg-white">
                          {relatedCover ? (
                            <Image
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="line-clamp-2 text-sm font-bold leading-5 text-slate-950">{relatedItem.title}</p>
                          <p className="mt-1 text-xs font-black text-blue-600">{formatProductPrice(relatedItem)}</p>
                        </div>
            ...
            <section className="min-w-0 rounded-[8px] border border-blue-100 bg-white p-4 shadow-xl shadow-blue-950/10 sm:p-5">
              <p className="text-xs font-bold uppercase text-blue-700">Detalhes rapidos</p>
        ...
        <section className="mt-4 rounded-[8px] border border-blue-100 bg-white p-5 shadow-xl shadow-blue-950/10 sm:p-7">
          <details id="descricao-completa" className="group">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
              <span>
                <span className="block text-xs font-bold uppercase text-blue-700">Descricao completa</span>
                <span className="mt-1 block text-lg font-black text-slate-950">Tudo sobre este produto</span>
              </span>
      ...
      <MobileProductCheckoutBar priceLabel={priceLabel} productId={item.id} disabled={!canCheckout} />
...
function TrustPill({ icon, label }: { icon: ReactNode; label: string }) {
  return (
    <span className="inline-flex min-h-9 items-center gap-2 rounded-full border border-blue-100 bg-blue-50 px-3 text-xs font-bold text-blue-700">
      {icon}
      <span>{label}</span>
    </span>
  );
...
function MobileProductCheckoutBar({
  disabled,
  priceLabel,
  productId,
}: {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-blue-100 bg-white/95 px-4 py-3 shadow-[0_-16px_40px_rgba(15,23,42,0.14)] backdrop-blur sm:hidden">
      <div className="mx-auto grid max-w-md grid-cols-[minmax(0,1fr)_164px] items-center gap-3">
        <div className="min-w-0">
          <p className="text-xs font-bold uppercase text-blue-700">Total</p>
          <p className="truncate text-xl font-black text-slate-950">{priceLabel}</p>
        </div>
        <ProductCheckoutButton productId={productId} disabled={disabled} />
      </div>
    </div>
  );
}
...
function StoreIdentity({ branding }: { branding: OrganizationBranding }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        <p className="text-xs font-bold uppercase text-[#128C4A]">Loja oficial</p>
