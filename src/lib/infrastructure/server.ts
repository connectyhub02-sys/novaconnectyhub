...
export async function migrationCatalog(project: string): Promise<Migration[]> {
    ...
    const prepared = (data ?? []).map(m => ({ ...m, risk: migrationRisk(m.sql), blockers: executionPolicy(m.sql) }));
    if (project !== "connectyhub") return prepared;
  const directory = join(process.cwd(), "supabase", "migrations");
  const files = (await readdir(directory)).filter(f => /^\d+_[a-zA-Z0-9_-]+\.sql$/.test(f)).sort();
  const versioned = await Promise.all(files.map(async name => {
    const sql = (await readFile(join(directory, name), "utf8")).replace(/\r\n/g, "\n");
    return { version: name.split("_")[0], name, sql, checksum: createHash("sha256").update(sql).digest("hex"), risk: migrationRisk(sql), blockers: executionPolicy(sql) };
  }));
  return [...versioned, ...prepared.filter(m => !versioned.some(v => v.version === m.version))].sort((a, b) => a.version.localeCompare(b.version));
}
...
export function migrationExecution(project: string): MigrationExecution {
  const hasProjectUrl = !!projectDatabaseUrl(project);
  const missing = [
