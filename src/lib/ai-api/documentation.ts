...
import {aiCookbookPages} from './cookbook';
import {aiIntegrationGuides} from './integration-guides';
import {aiHttpReferencePages} from './api-reference';
import {aiReadinessPages} from './readiness-documentation';
...
export const aiDocPages: AiDocPage[] = [
  ...
  { id: "ia-chat", label: "Gerar resposta", group: "Endpoints", method: "POST", path: "/chat/completions", title: "Gerar uma resposta", description: aiOpenApiSpec.paths["/chat/completions"].post.description, blocks: [
    ...
    table("Campos da resposta", ["Campo", "Significado"], [["id", "Identificador da resposta, com prefixo chatcmpl-."], ["object", "chat.completion."], ["created", "Data Unix em segundos."], ["model", "ID público do modelo utilizado pela chave."], ["choices[0].message", "role=assistant e content com o texto."], ["choices[0].finish_reason", "stop: concluída; length: resposta parcial; content_filter: sem texto disponível."], ["connectyhub", "request_id, project_id e credits da operação."]]),
  ...
  { id: "ia-stream", label: "Eventos SSE", group: "Implementar", title: "Receber a resposta como eventos", description: "stream=true entrega partes durante a geração. Os créditos são confirmados no encerramento.", blocks: [
    ...
    table("Eventos", ["Ordem", "Conteúdo", "Ação do cliente"], [["1..N", "choices[0].delta.role, delta.content ou delta.tool_calls", "Acumule as partes na ordem recebida."], ["Final", "delta vazio, finish_reason e connectyhub", "Confira a conclusão e os créditos; guarde request_id."], ["Opcional", "choices vazio e usage", "Presente quando stream_options.include_usage=true."], ["Término", "data: [DONE]", "Finalize a leitura."]]),
    note("Escolha do protocolo", "Chat Completions e /models/{model}:streamGenerateContent entregam geração incremental. A base /api/v1beta tem o formato Gemini, sem [DONE]. Para áudio bidirecional, use /live quando habilitado. Uma reconexão idempotente pode entregar a resposta já concluída em um único trecho; não repete a geração."),
  ...
  { id:"ia-conteudo",label:"Conteúdo multimodal",group:"Endpoints",method:"POST",path:"/models/{model}:generateContent",title:"Conteúdo, mídia e configurações",description:"Use a estrutura contents/parts para controlar mensagens, mídia e ferramentas com uma resposta textual.",blocks:[
    ...
    note("Entrega em eventos", "POST /models/{model}:streamGenerateContent entrega partes incrementais por SSE, seguidas de content.completed com os créditos confirmados e [DONE]. Partes recebidas antes da conclusão são provisórias. Chat Completions com stream=true também entrega partes incrementais."),
  ]},
  { id:"ia-arquivos",label:"Arquivos e PDF",group:"Endpoints",method:"POST",path:"/files",title:"Enviar e reutilizar arquivos",description:"Arquivos pertencem ao projeto; uma chave de outro projeto não pode consultá-los nem usá-los.",blocks:[
    ...
    steps("Analisar um arquivo",["Para arquivos pequenos, envie data em base64 puro, mime_type e display_name, respeitando o limite de 4 MB do JSON. Arquivos de até 20 MB dependem do relay; consulte a seção Tarifas e limites.","Guarde id e name da resposta. Consulte GET /files/{id} até status=active; processing ainda não está pronto.","Envie name em fileData.fileUri junto com sua pergunta para generateContent.","Observe expires_at; arquivos expirados precisam ser enviados novamente. Exclua com DELETE /files/{id} quando terminar."]),
