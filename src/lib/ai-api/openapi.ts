...
import {readinessAiPaths,readinessAiSchemas} from './readiness-openapi';
...
export const aiOpenApiSpec = {
  openapi: "3.1.0",
  info: { title: "ConnectyHub API de IA", version: "1.6.0", description: "Crie um projeto, copie a chave e conecte seu sistema. Escolha o modelo por chave. A API gera respostas, analisa arquivos e produz vetores, com consumo em créditos ConnectyHub. Documentação e JSON públicos; uso requer projeto ativo, acesso e saldo disponível. Exemplos de créditos são ilustrativos. A resposta utiliza um subconjunto de Chat Completions e informa cobrança em connectyhub.credits; metadados de consumo estão disponíveis no contrato Gemini e no SSE opcional." },
  ...
  paths: {
    ...
    "/chat/completions": { post: {
      ...
      responses: {
        "200": { description: "Resposta concluída ou recuperada. stream=true entrega SSE incremental durante a geração, com créditos no evento final e data: [DONE].", headers: { "X-Request-Id": { description: "UUID para consultar a solicitação", schema: { type: "string", format: "uuid" } }, "Idempotency-Replayed": { description: "Indica resultado recuperado sem nova execução", schema: { type: "string", enum: ["true","false"] } } }, content: { "application/json": { schema: ref("ChatCompletion"), example: aiResponseExample }, "text/event-stream": { schema: { type: "string" }, example: aiSseExample, "x-event-schema": ref("ChatChunk"), description: "Eventos chat.completion.chunk com deltas de texto/ferramentas, encerramento com connectyhub.credits e data: [DONE]. include_usage adiciona evento com choices vazio e usage. Erro após HTTP 200 aparece no stream." } } },
  ...
  components: {
    ...
    schemas: {
      ...
      ChatChunk: { type: "object", required: ["id", "object", "created", "model", "choices"], properties: {
        ...
        choices: { type: "array", minItems: 0, maxItems: 1, items: { type: "object", required: ["index", "delta", "finish_reason"], properties: {
        ...
        usage: {type:'object',description:'Evento opcional de consumo com choices vazio.',properties:{prompt_tokens:{type:'integer'},completion_tokens:{type:'integer'},total_tokens:{type:'integer'}}},
      } },
