export type PaymentDiagnostic = {
  ...
  stage: "customer_lookup" | "customer_create" | "payment_create" | "reconcile";
  code: string;
  httpStatus: number | null;
};

const allowedCodes = new Set(["invalid_creditCard", "invalid_creditCardHolderInfo", "invalid_cpfCnpj", "invalid_customer", "invalid_value", "invalid_installmentCount", "invalid_access_token", "access_token_not_found", "invalid_object", "invalid_action", "invalid_payment"]);
...
export function classifyAsaasFailure(status: number | null, endpoint: string, method: string, body?: unknown): PaymentDiagnostic {
  const stage = endpoint.startsWith("/customers") ? method === "POST" ? "customer_create" : "customer_lookup" : method === "POST" && ["/payments", "/subscriptions"].includes(endpoint) ? "payment_create" : "reconcile";
