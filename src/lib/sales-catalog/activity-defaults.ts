...
import { activityDefaultDestination, activityDeliveryProfile, activityLabel } from "@/lib/whatsapp/activity-profile";
...
export async function loadCatalogActivityDefaults(client: SupabaseClient, companyId: string, agentId?: string | null) {
  ...
  if (agents.length !== 1) return { destination: "connectyhub_checkout" as const, activity: null, activityLabel: null, delivery: "livre" as const };
  ...
  return { destination: enabled ? destination : "manual_handoff" as const, activity: config.templateId,
    activityLabel: activityLabel(config.templateId), delivery: activityDeliveryProfile(config.templateId) };
}
