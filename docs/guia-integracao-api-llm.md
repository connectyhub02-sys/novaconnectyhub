...
| Lotes | POST models/{model}:batchGenerateContent / :asyncBatchEmbedContent; GET/DELETE batches/{id}; POST batches/{id}:cancel | Até 100 itens inline; uma reserva por lote. Exclusão só após encerramento. Entrada por arquivo, filtros, prioridade e webhooks ainda pendentes. |
| File Search | /api/v1beta/fileSearchStores; :importFile; /documents; /operations | Coleções e documentos isolados por projeto; listas até 20 itens com estado persistido. Consulte cada recurso para atualizar. Upload nativo do SDK e embeddingModel personalizado ainda não suportados. |
| Interações | Contrato ConnectyHub /api/v1/ai/interactions | Não é substituição transparente de todos os métodos do SDK Gemini. |
