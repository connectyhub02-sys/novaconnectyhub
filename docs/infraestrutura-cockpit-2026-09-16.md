...
- Preparação de aplicar migration, pausar/retomar função, reprocessar/reenviar evento e rollback: confirmação vinculada a projeto/ação/alvo; migration também vinculada ao checksum atual. **Todos os comandos remotos ficam bloqueados (501), inclusive para admin infra**, pois os adaptadores não estão habilitados. O painel informa o motivo antes da confirmação; confirmar registra uma solicitação bloqueada, sem executar.
