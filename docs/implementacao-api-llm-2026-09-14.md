...
## Publicação corretiva confirmada — 14/09, 13:09 UTC

Commit 508953c na master; implantação dpl_bgU48TSxbC7MFvu45XdHe92h2G5V Ready/Production e domínio principal confirmado. Logs de build confirmaram o commit, compilação Turbopack e TypeScript aprovados; publicação concluída às 13:07:45 UTC. Quatorze probes públicos aprovados: OpenAPI/guia/preços/login 200, dez requisições protegidas com 401 e motivo invalid_api_key, incluindo consultas de arquivos/lotes/coleções, criação e upload. Três dessas consultas usaram chave sintética deliberadamente inválida, sem concessão de acesso. O 503 anterior não se repetiu.

[Evidência das verificações finais](evidencias/gemini-publicacao-complementos-2026-09-14.json). Nenhuma geração paga, indexação real, arquivo pessoal, recarga ou alteração de saldo para testar. Os fluxos publicados estão disponíveis para testes controlados; compatibilidade integral e homologação paga de cada modalidade não estão concluídas. Este registro de verificação fica local até o próximo pacote para evitar um deploy adicional apenas documental. Nenhum processo local de teste ou build permanece ativo nesta tarefa.
