...
Revisão 3, preparação local para host compartilhado: requisito de daemon vazio substituído por PostgreSQL com network_mode none e demais serviços no mesmo namespace privado, somente loopback. Sem bridge/firewall/portas do host. Labels UUID, escopo de mounts, snapshots hash dos containers preexistentes e recusa de namespace/consumidor estrangeiro implementados. Storage limitado não exige disco/partição: novo módulo provisiona arquivo ext4 prealocado de 2 GiB, loop e mount exclusivos com verificação de identidade e release/discard de escopo estrito. Nenhum desses comandos Linux foi executado.

13 testes locais aprovados sem skips, incluindo novamente os 107 checks HTTP nativos e restauração real; após reforçar guardas de PID/logs/sem-download, 11 regressões curtas passaram (PG já coberto na rodada completa). Prévia fictícia reaberta e preservada. Imagens agora devem estar preparadas: run não faz pull/build no daemon compartilhado, pois limites dos serviços não contêm esses consumos. Permanecem limites RAM/CPU/PIDs/logs por container, 2.304 MiB/2,75 CPUs conservadores, prazo de 45 minutos, e reserva de 256 MiB de host sem cgroup próprio. I/O não tem taxa máxima imposta; requer janela de baixa carga.
...
Corrigidos no pacote de ensaio: descarte de blocos pelo mkfs, prontidão do PostgreSQL temporário, permissões do código na imagem, alias do teste de pressão, cópia para diretório vazio precriado e inspeção de containers efêmeros durante down/restore. A consulta compatível de estrangeiros seleciona IDs por label antes do inspect, preservando a comparação rígida de produção. 14 regressões locais passaram; um teste PostgreSQL local foi omitido nessa rodada curta, com execução Linux real concluída.
...
## Betel: operação nova habilitada — 15/09/2026, 20:29 UTC

Substitui o HOLD global anterior: broker com 12 funções ativo, webhooks novos encaminhados, pausas comerciais desligadas e fornecedor WhatsApp liberado. Histórico incerto permanece retido individualmente, sem replay. App inicial 74cbb699, atualizado para a3111e66 às 20:57 UTC; portal atualizado e navegação autenticada conferida. Passaram 27 testes do broker,8 do inbox,40 HTTP Betel e69 CH. SDR/grupos tiveram resultados funcionais positivos com filas vazias; o cron legado scraper é no-op intencional, Meta fica para outra etapa.

Incidente da publicação: quatro serviços API/Auth existentes ficaram parados e foram retomados às 20:34, sem recriar banco. Usar o guard de dependências após próximas publicações. A API do histórico WhatsApp retornou404 instance_not_found às 20:45; a causa eram três instâncias arquivadas. O filtro foi corrigido/publicado; leitura da instância atual passou, mas ainda falta observar a próxima sincronização natural. Coleta do portal voltou a passar depois de falha pontual de causa não confirmada. Backup/configuração operacional têm cópia privada fora da VPS; não equivalem a backup externo recorrente. [Relatório, evidências e limites](betel-ativacao-operacional-2026-09-15.md).

## Betel: lote real e ensaio retirado — 15/09/2026, 21:47 UTC

Ensaio `betel-ai-rehearsal` removido com autorização, broker sintético parado/desabilitado e evidência privada copiada fora da VPS. Preservadas as 12 funções Betel e 45 ConnectyHub; não recriar o ensaio.

Broker corrigido/publicado às 21:34 UTC: diferencia identificador externo do evento e recibo interno do motor, preservando assinatura e correspondência exata de payload. 29 testes passaram. Recuperação única autorizada do lote de cinco links, após provar zero efeitos. Motor encerrou FAILED após ~301s, mas aplicação continuou e persistiu cinco coletas: dois resultados parciais para revisão e três falhas de extração. Nenhum replay após começar os efeitos; não repetir o lote pelo status técnico. Divisão em etapas por link ainda é trabalho do app Betel, fora desta publicação.

Conciliação de quatro operações IA: três completed somam 119.751120 créditos nessa amostra; uma uncertain conserva reserva de 517.623600 créditos e precisa resolução. Não confundir resposta técnica completed com análise útil nem liberar/repetir operação incerta. Supabase Studio respondeu HTTP200 com credencial própria, entregue em arquivo privado; login no navegador ainda não confirmado. [Relatório completo e evidências](betel-lote-real-e-retirada-ensaio-2026-09-15.md).
