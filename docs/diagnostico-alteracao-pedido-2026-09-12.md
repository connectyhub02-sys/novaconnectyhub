...
A leitura posterior confirmou as cinco funções novas com hashes iguais à migration, os três gatilhos habilitados, o índice de exclusão de concorrência válido e RLS ativada. A tabela e as RPCs não têm acesso para `anon` ou `authenticated`; o servidor mantém os privilégios necessários. Recarga de schema solicitada.

Aplicação publicada na master em `b99024c90f0b80ccef833c1efeaeb2d1788dfb5e`. Vercel confirmou a implantação `dpl_85E11Du5v36SA32C37qUw5hQ1FwJ` em **Ready / Latest / Production**, com domínio principal vinculado e término às 18h04 BRT de 12/09; duração de build de 55 segundos. Conferência pública posterior: página inicial e login HTTP 200, handler Inngest HTTP 200 com assinatura aceita e 43 funções; consulta de checkout inexistente retornou 404 esperado. Nenhuma mensagem, pedido, cobrança ou revisão real foi criada para esses testes. O titular pode realizar o reteste do atendimento; a publicação não substitui essa evidência funcional real.

Esclarecimento durante a publicação: o SQL Editor da VPS estava sem consultas salvas na barra lateral, enquanto o banco tinha 153 tabelas no esquema `public` e 86 migrations registradas. Scripts/consultas salvos na interface não são as tabelas nem o histórico de migrations. Foi mostrada a nova tabela no Table Editor; nenhuma recuperação de consultas salvas do painel Cloud foi executada. Registro pós-publicação mantido local para o próximo conjunto documental, sem disparar outra implantação apenas por documentação.
